...
def task_format_code():
    ...
    result = subprocess.run(
        ["black", "--check"] + PROJECT_FILES, capture_output=True, text=True
    )
    if result.returncode != 0:
        print(
            "[WARNING] Le code n'est pas parfaitement formaté selon les standards Black."
        )
        print(result.stderr)
    else:
...
def task_lint_code():
    ...
    result = subprocess.run(
        ["flake8", "--max-line-length=120"] + PROJECT_FILES,
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
...
def task_run_tests():
    ...
    result = subprocess.run(
        ["pytest", "test_pipeline.py"], capture_output=True, text=True
    )
    print(result.stdout)
    if result.returncode != 0:
...
@task(name="Code - 5. Lancement de l'API FastAPI")
def task_launch_api(host: str = "0.0.0.0", port: int = 8000):
    print(f"[API] Lancement de FastAPI sur http://{host}:{port} ...")
    subprocess.run(
        [
            sys.executable,
            "-m",
...
@task(name="2. Configurer MLflow")
def task_configure_mlflow():
    main.model_pipeline.configure_mlflow()


@task(name="3. Préparation des Données")
def task_prepare_data(data_path: str = "Churn_Modelling.csv"):
    return main.run_preparation(data_path)


@task(name="4. Entraînement du Modèle")
def task_train_model(X_train, y_train):
    return main.run_training(X_train, y_train)


@task(name="5. Sauvegarder le Modèle")
def task_save_model(
    model, scaler, model_path: str = "svc_model.pkl", scaler_path: str = "scaler.pkl"
):
    main.run_saving(model, scaler, model_path, scaler_path)


@task(name="6. Charger le Modèle")
...
@flow(
    name="code",
    description="Flow dédié à la qualité, au formatage, à la sécurité et aux tests du code.",
)
...
@flow(
    name="all",
    description="Flow complet exécutant le suivi du code suivi du pipeline ML complet.",
)
def all_pipeline_flow(data_path: str = "Churn_Modelling.csv"):
    print("=== [FLOW] Début du Flow GLOBAL: all ===")

    # Configuration MLflow
    task_configure_mlflow()
...
@flow(
    name="entrainement",
    description="Flow de préparation des données et d'entraînement du modèle.",
)
...
@flow(name="api", description="Flow de lancement de l'API FastAPI.")
def api_pipeline_flow(host: str = "0.0.0.0", port: int = 8000):
    print("=== [FLOW] Début du Flow: api ===")
    task_launch_api(host=host, port=port)
    print("=== [FLOW] Fin du Flow: api ===")
...
def parse_and_run():
    parser = argparse.ArgumentParser(
        description="Automatisation des Flows Prefect (CI/CD et ML Pipeline)."
    )
    parser.add_argument(
        "--flow",
        type=str,
        required=True,
        choices=["all", "entrainement", "evaluate", "code", "api"],
        help="Nom du flow Prefect à exécuter (all, entrainement, evaluate, code, api)",
    )
    parser.add_argument(
        "--data",
        type=str,
